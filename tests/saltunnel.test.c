...
static void* saltunnel_thread_inner(void* v)
{
    ...
    saltunnel_thread_context* c = v;
    log_set_thread_name(c->thread_name);
    saltunnel_mx(c->ingress, c->egress);
...
static pthread_t saltunnel_thread(const char* thread_name, cryptostream* ingress, cryptostream* egress)
{
    ...
    c->egress = egress;
    pthread_t thread;
    pthread_create(&thread, NULL, saltunnel_thread_inner, c)==0 || oops_error("pthread_create failed");
    return thread;
}
...
static pthread_t write_thread(const char* thread_name, int fd,const char *buf,unsigned int len)
{
    ...
    c->len = len;
    pthread_t thread;
    pthread_create(&thread, NULL, write_thread_inner, c)==0 || oops_error("pthread_create failed");
    return thread;
}
