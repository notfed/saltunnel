...
static int tcp_socket_new(const char* ip, const char* port)
{
    // Resolve address
    ...
    log_info("socket created, fd %d", s);
    if (s == -1) return -1;
    // Enable TCP_NODELAY
...
static int tcp_connection_new(int s) {
    ...
    log_info("connection created, fd %d", fd_conn);
...
int saltunnel_tcp_server(const char* from_ip, const char* from_port,
                         const char* to_ip, const char* to_port)
{
    ...
    // Create socket
    int s = tcp_socket_new(from_ip, from_port);
    ...
    for(;;) {
        // Accept a new connection
        int fd_conn = tcp_connection_new(s);
