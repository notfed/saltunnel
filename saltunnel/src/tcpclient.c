...
int tcpclient_new(const char* ip, const char* port, tcpclient_options options)
{
    ...
    if (s<0) return cleanup_oops_warn(s, "failed to create TCP client socket");
    ...
    if(options.OPT_TCP_NODELAY) {
        ...
        if(setsockopt(s, IPPROTO_TCP, TCP_NODELAY, &historical_api_flag, sizeof(int))<0)
            return cleanup_oops_warn(s, "failed to enable TCP_NODELAY on TCP client socket");
    }
    ...
    if(options.OPT_SO_SNDLOWAT>0) {
        ...
        if(setsockopt(s, SOL_SOCKET, SO_SNDLOWAT, &low_water_mark_bytes, sizeof(int))<0)
            log_info("failed to set SO_SNDLOWAT on TCP client socket"); // "Not changeable on Linux"
    }
    ...
    if(connect(s, (struct sockaddr*)&server_address, sizeof(server_address))<0)
        return cleanup_oops_warn(s, "failed to connect to TCP server");
    
    // Make it non-blocking
    if(options.OPT_NONBLOCK) {
        if(fd_nonblock(s)<0)
            return cleanup_oops_warn(s, "failed to set set O_NONBLOCK on TCP client connection");
    }
