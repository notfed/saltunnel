...
int decrypt_one(int buffer_i, cryptostream *cs, nonce8 nonce) {
    ...
    // - signature: crypto_secretbox8_open(m,c,clen,n,k)
    // - input structure:
    //   - [0..16]  == zero
    //   - [16..32] == auth
    //   - [32..]   == ciphertext
    ...
    //   - [0..32] == zero
    //   - [32..]  == plaintext
    if(crypto_secretbox8_open(plaintext_buffer_ptr, ciphertext_buffer_ptr,
                              CRYPTOSTREAM_BUFFER_MAXBYTES, nonce, cs->key)<0)
    { errno = EBADMSG; return oops("decryption failed: MAC validation failed"); }
    
    // Extract datalen
    uint16_t datalen_current = 0;
    uint16_unpack((char*)plaintext_buffer_ptr + 32, &datalen_current);
    if(datalen_current>494)
    { errno = EBADMSG; return oops("decryption failed: invalid length"); }
