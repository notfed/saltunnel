...
//  saltunnel
//
//  A cryptostream acts  encapsulates one "stream" of of data within a
//  saltunnel session. (Each direction of communication is a stream.)
//
//  A stream may either be an "encrypt" stream, or a "decrypt" stream.
//
//  An "encrypt stream" reads plaintext from its "from" file descriptor,
//  encrypts chunk-by-chunk, and writes chunks to its "to" file descriptor.
//
//  An "decrypt stream" reads plaintext from its "from" file descriptor,
//  decrypts chunk-by-chunk, and writes chunks to its "to" file descriptor.
//
//  The primary data pumps of both streams are the "feed" functions:
//  "feed_canread", "feed_read", "feed_canwrite", "feed_write". All other
//  functions are called from these.
//
...
typedef struct __attribute__((aligned(32))) cryptostream {
