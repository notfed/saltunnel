...
int encrypt_one(int buffer_i, int buffer_n, int bytesread, cryptostream *cs, nonce8 nonce) {
    ...
    memset(plaintext_buffer_ptr+32+2+current_bytes_to_encrypt, 0, CRYPTOSTREAM_BUFFER_MAXBYTES_DATA-current_bytes_to_encrypt);
    
    // Encrypt chunk from plaintext to ciphertext (494 bytes)
    
    // crypto_secretbox:
    // - signature: crypto_secretbox8(c,m,mlen,n,k);
    // - input structure:
    //   - [0..32] == zero
    //   - [32..]  == plaintext
    // - output structure:
    ...
    //   - [16..32] == auth
    //   - [32..]   == ciphertext
    if(crypto_secretbox8(ciphertext_buffer_ptr, plaintext_buffer_ptr,
